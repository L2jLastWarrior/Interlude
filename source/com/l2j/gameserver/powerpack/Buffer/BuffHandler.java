...
public class BuffHandler implements IVoicedCommandHandler, ICustomByPassHandler, IBBSHandler
{
	...
	{
		return new String[] {Config.BUFFER_COMMAND};
	}
	...
	private boolean checkAllowed(L2PcInstance activeChar)
	{
		...
		else if(Config.BUFFER_EXCLUDE_ON.contains("ALL"))
			msg = "Buffer is not available in this area";
		else if(Config.BUFFER_EXCLUDE_ON.contains("CURSED") && activeChar.isCursedWeaponEquiped())
			msg = "Can't use Buffer with Cursed Weapon"; 
		else if(Config.BUFFER_EXCLUDE_ON.contains("ATTACK") && AttackStanceTaskManager.getInstance().getAttackStanceTask(activeChar))
			msg = "Buffer is not available during the battle";
		else if(Config.BUFFER_EXCLUDE_ON.contains("DUNGEON") && activeChar.isIn7sDungeon())
			msg = "Buffer is not available in the catacombs and necropolis";
		else if(Config.BUFFER_EXCLUDE_ON.contains("RB") && activeChar.isInsideZone(L2Character.ZONE_NOSUMMONFRIEND))
			msg = "Buffer is not available in this area";
		else if(Config.BUFFER_EXCLUDE_ON.contains("PVP") && activeChar.isInsideZone(L2Character.ZONE_PVP))
			msg = "Buffer is not available in this area";
		else if(Config.BUFFER_EXCLUDE_ON.contains("PEACE") && activeChar.isInsideZone(L2Character.ZONE_PEACE))
			msg = "Buffer is not available in this area";
		else if(Config.BUFFER_EXCLUDE_ON.contains("SIEGE") && activeChar.isInsideZone(L2Character.ZONE_SIEGE))
			msg = "Buffer is not available in this area";
		else if(Config.BUFFER_EXCLUDE_ON.contains("OLYMPIAD") && (activeChar.isInOlympiadMode() ||
			...
			String target)
	{
		...
		if(command.compareTo(Config.BUFFER_COMMAND)==0)
		{
			NpcHtmlMessage htm = new NpcHtmlMessage(activeChar.getLastQuestNpcObject());
			String text = HtmCache.getInstance().getHtm("data/html/default/"+Config.BUFFER_NPC+".htm");
			htm.setHtml(text);
			...
			String parameters)
	{
		...
		if(!Config.BUFFER_USEBBS && !Config.BUFFER_USECOMMAND){
			
			if(player.getTarget()!=null)
				if(player.getTarget() instanceof L2NpcInstance)
				{
					buffer = (L2NpcInstance)player.getTarget();
					if(buffer.getTemplate().getNpcId()!=Config.BUFFER_NPC)
						buffer=null;
				}
		...
		else if(parameters.startsWith("Chat"))
		{
			...
			if(player.getAdena()<Config.BUFFER_PRICE*3) {
			...
			player.reduceAdena("Buff", Config.BUFFER_PRICE*3, null, true);
		}
			...
			player.getStatus().setCurrentCp(player.getMaxCp());
			player.reduceAdena("Buff", Config.BUFFER_PRICE, null, true);
		}
		...
		else if(parameters.startsWith("MakeBuffs") || parameters.startsWith("RestoreBuffs"))
		{
			...
			if(_visitedPages.get(player.getObjectId())!=null)
				...
				useVoicedCommand(Config.BUFFER_COMMAND, player, "");
		...
		else if (currentCommand.startsWith("skill"))
		{
			...
			if (currentCommand.startsWith("skillselect") && !scheme_key.equalsIgnoreCase("unselected"))
			{
				if(CharSchemesTable.getInstance() != null
						...
						&& CharSchemesTable.getInstance().getScheme(player.getObjectId(), scheme_key).size() < Config.NPCBUFFER_MAX_SKILLS)
		...
		else if (currentCommand.startsWith("fighterbuff") || currentCommand.startsWith("magebuff"))
		{
			...
			if(currentCommand.startsWith("magebuff")){
				...
				for(int skillId:Config.MAGE_SKILL_LIST.keySet()){
					
					L2Skill skill = SkillTable.getInstance().getInfo(skillId, Config.MAGE_SKILL_LIST.get(skillId));
					if(skill!=null)
					{
				...
				for(int skillId:Config.FIGHTER_SKILL_LIST.keySet()){
					
					L2Skill skill = SkillTable.getInstance().getInfo(skillId, Config.FIGHTER_SKILL_LIST.get(skillId));
					if(skill!=null)
					{
						skills_to_buff.add(skill);
					}
			...
			int cost = 0;
			if(Config.BUFFER_PRICE>0)
				cost = Config.BUFFER_PRICE*skills_to_buff.size();

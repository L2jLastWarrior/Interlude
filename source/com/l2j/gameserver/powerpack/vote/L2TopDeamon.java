package com.l2j.gameserver.powerpack.vote;
...
public class L2TopDeamon implements Runnable
{
			...
			try
			{
				...
				if (sendPrize)
				{
					...
					if (player != null)
					{
						...
						/*if ((player.isOnline() != 0) && (Config.L2TOPDEMON_MESSAGE != null) && (Config.L2TOPDEMON_MESSAGE.length() > 0))
						{
	...
	private boolean checkVotes()
	{
		...
		try
		{
			...
			int nVotes = 0;
			URL url = new URL(Config.L2TOPDEMON_URL);
			is = url.openStream();

package com.l2j.gameserver.powerpack.engrave;
...
import com.l2j.gameserver.model.quest.State;
...
public class EngraveQuest extends Quest
{
	...
	public EngraveQuest()
	{
		...
		if(Config.SPAWN_ENGRAVER)
		{
			addSpawn(50018, Config.ENGRAVER_X, Config.ENGRAVER_Y, Config.ENGRAVER_Z, 0, false, 0);
	...
	public String onAdvEvent(String event, L2NpcInstance npc, L2PcInstance player)
	{
		...
		if(event.startsWith("mark") || event.startsWith("clear"))
		{
			...
			if(it != null)
			{
				if(Config.ENGRAVE_PRICE > 0 && Config.ENGRAVE_PRICE_ITEM > 0 && event.startsWith("mark"))
				{
					L2ItemInstance pit = player.getInventory().getItemByItemId(Config.ENGRAVE_PRICE_ITEM);
					if(pit == null || pit.getCount() < Config.ENGRAVE_PRICE)
					{
						htm = HtmCache.getInstance().getHtm("data/html/default/50018-6.htm");
						htm = htm.replace("%itemname%", ItemTable.getInstance().getTemplate(Config.ENGRAVE_PRICE_ITEM).getName());
						htm = htm.replace("%count%", String.valueOf(Config.ENGRAVE_PRICE));
						return htm;
					}
					player.destroyItemByItemId("use", Config.ENGRAVE_PRICE_ITEM, Config.ENGRAVE_PRICE, npc, true);
				}
		...
		{
			if(Config.MAX_ENGRAVED_ITEMS_PER_CHAR != 0)
			{
				if(EngraveManager.getInstance().getMyEngravement(player).size() >= Config.MAX_ENGRAVED_ITEMS_PER_CHAR)
				{
					htm = HtmCache.getInstance().getHtm("data/html/default/50018-7.htm");
					htm = htm.replace("%cnt%", String.valueOf(Config.MAX_ENGRAVED_ITEMS_PER_CHAR));
					return htm;
				}
			}
			int iPos = event.lastIndexOf("_");
			...
			if(Config.ENGRAVE_PRICE > 0 && Config.ENGRAVE_PRICE_ITEM > 0)
			{
				caption += "<br1>It will cost you <font color=\"LEVEL\">" + Config.ENGRAVE_PRICE + " " + ItemTable.getInstance().getTemplate(Config.ENGRAVE_PRICE_ITEM).getName() + "</font>";

				L2ItemInstance it = player.getInventory().getItemByItemId(Config.ENGRAVE_PRICE_ITEM);
				if(it == null || it.getCount() < Config.ENGRAVE_PRICE)
				{
					htm = HtmCache.getInstance().getHtm("data/html/default/50018-6.htm");
					htm = htm.replace("%itemname%", ItemTable.getInstance().getTemplate(Config.ENGRAVE_PRICE_ITEM).getName());
					htm = htm.replace("%count%", String.valueOf(Config.ENGRAVE_PRICE));
			...
			htm = htm.replace("%list%", buildList(player, startwith, "engrave", "mark", new CondChecker() {
				...
				public boolean check(L2ItemInstance item, L2PcInstance player)
				{
					synchronized (Config.ENGRAVE_EXCLUDED_ITEMS)
					{

						return !item.isEquipped() && item.isEquipable() && !item.isShadowItem() && !EngraveManager.getInstance().isEngraved(item.getObjectId()) && !Config.ENGRAVE_EXCLUDED_ITEMS.contains(item.getItemId()) && Config.ENGRAVE_ALLOW_GRADE.contains(item.getItem().getCrystalType());
					}
				}
			}));
		}

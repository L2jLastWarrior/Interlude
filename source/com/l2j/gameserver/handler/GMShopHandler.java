...
public class GMShopHandler implements IVoicedCommandHandler, ICustomByPassHandler, IBBSHandler
{
	...
	public String[] getVoicedCommandList()
	{
		...
		return new String[] {Config.GMSHOP_COMMAND};
	}
	...
	private boolean checkAllowed(L2PcInstance activeChar)
	{
		...
		else if(Config.GMSHOP_EXCLUDE_ON.contains("ALL"))
			msg = "GMShop is not available in this area";
		else if(Config.GMSHOP_EXCLUDE_ON.contains("CURSED") && activeChar.isCursedWeaponEquiped())
			msg = "GMShop is not available with the cursed sword"; 
		else if(Config.GMSHOP_EXCLUDE_ON.contains("ATTACK") && AttackStanceTaskManager.getInstance().getAttackStanceTask(activeChar))
			msg = "GMShop is not available during the battle";
		else if(Config.GMSHOP_EXCLUDE_ON.contains("DUNGEON") && activeChar.isIn7sDungeon())
			msg = "GMShop is not available in the catacombs and necropolis";
		else if(Config.GMSHOP_EXCLUDE_ON.contains("RB") && activeChar.isInsideZone(L2Character.ZONE_NOSUMMONFRIEND))
				msg = "GMShop is not available in this area";
		else if(Config.GMSHOP_EXCLUDE_ON.contains("PVP") && activeChar.isInsideZone(L2Character.ZONE_PVP))
			msg = "GMShop is not available in this area";
		else if(Config.GMSHOP_EXCLUDE_ON.contains("PEACE") && activeChar.isInsideZone(L2Character.ZONE_PEACE))
			msg = "GMShop is not available in this area";
		else if(Config.GMSHOP_EXCLUDE_ON.contains("SIEGE") && activeChar.isInsideZone(L2Character.ZONE_SIEGE))
			msg = "GMShop is not available in this area";
		else if(Config.GMSHOP_EXCLUDE_ON.contains("OLYMPIAD") && (activeChar.isInOlympiadMode() ||
	...
	public boolean useVoicedCommand(String command, L2PcInstance activeChar, String params)
	{
		...
		if(command.compareTo(Config.GMSHOP_COMMAND)==0)
		{
			String index = "";
	...
	public void handleCommand(String command, L2PcInstance player, String parameters)
	{
		if(player==null) 
	 	...
	 	if(!Config.GMSHOP_USEBBS && !Config.GMSHOP_USECOMMAND){
			...
			if(player.getTarget()!=null)
				if(player.getTarget() instanceof L2NpcInstance)
				{
					...
					if(gmshopnpc.getTemplate().getNpcId()!=Config.GMSHOP_NPC)
						gmshopnpc=null;
				}
